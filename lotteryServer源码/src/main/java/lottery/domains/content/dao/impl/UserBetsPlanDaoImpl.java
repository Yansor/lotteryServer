...
public class UserBetsPlanDaoImpl implements UserBetsPlanDao {
	...
	@Override
	public boolean updateFollow(int id, int count, double money,int part) {
		String hql = "update " + tab + " set followCount = followCount + ?1, rewardMoney = rewardMoney + ?2, part = part +?3 where id = ?0";
		Object[] values = {id, count, money,part};
