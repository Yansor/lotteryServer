...
public class HibernateSuperReadDao<T> {

	private static final Logger logger = LoggerFactory.getLogger(HibernateSuperReadDao.class);
	 ...
	 */
	@Transactional(readOnly = true)
	public PageList findPageList(final String queryHql, final String countHql, final Object[] values, final int start, final int limit) {
		try {
			PageList pList = new PageList();
			// 查询总数
			...
			if (values != null && values.length > 0) {
				for (int i = 0; i < values.length; i++) {
					countQuery.setParameter(String.valueOf(i), values[i]);
				}
			}
			...
			if (count > 0) {
				// 查询集合
				Query listQuery = getCurrentSession().createQuery(queryHql);

				if (values != null && values.length > 0) {
					for (int i = 0; i < values.length; i++) {
						listQuery.setParameter(String.valueOf(i), values[i]);
					}
				}
	 ...
	 */
	@Transactional(readOnly = true)
	public PageList findPageListByHQL(final String queryHql, final String countHql, final Map<String, Object> params, final int start, final int limit) {
		try {
			PageList pList = new PageList();
			// 查询总数
			Query countQuery = getCurrentSession().createQuery(countHql);
			if (params != null && params.size() > 0) {
				...
				for (String paramKey : paramKeys) {
					countQuery.setParameter(paramKey, params.get(paramKey));
				}
			}
			...
			if (count > 0) {
				...
				if (params != null && params.size() > 0) {
					Set<String> paramKeys = params.keySet();
					for (String paramKey : paramKeys) {
						listQuery.setParameter(paramKey, params.get(paramKey));
					}
				}
	 ...
	 */
	@Transactional(readOnly = true)
	public PageList findPageList(final String sql, Map<String, Object> params, final int start, final int limit) {
		try {
			PageList pList = new PageList();
			// 查询总数
	 ...
	 */
	@Transactional(readOnly = true)
	public PageList findPageList(final String countSql, final String querySql, Map<String, Object> params, final int start, final int limit) {
		try {
			PageList pList = new PageList();
			// 查询总数
			SQLQuery countQuery = getCurrentSession().createSQLQuery(countSql);
	 ...
	 */
	@Transactional(readOnly = true)
	public PageList findPageList(final Class<T> clazz, String propertyName, final List<Criterion> criterions, final List<Order> orders, final int start, final int limit) {
		try {
			PageList pList = new PageList();
