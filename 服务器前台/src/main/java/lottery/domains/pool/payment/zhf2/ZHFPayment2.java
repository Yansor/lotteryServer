// package lottery.domains.pool.payment.zhf2;
//
...
// import lottery.domains.pool.payment.zhf.DinpayXMLResult;
// import lottery.domains.pool.payment.zhf.ZHFPayment;
...
//  */
// public class ZHFPayment2 {
//     private static final Logger log = LoggerFactory.getLogger(ZHFPayment2.class);
...
//         try {
//             //商户公钥
//             sign = RSAWithSoftware.signByPrivateKey(signInfo, PayConfig.ZHF2.PRIVATE_KEY);
//         } catch (Exception e) {
//             e.printStackTrace();
//         }
//
...
//
//         result.setPayUrl(PayConfig.ZHF2.PAY_GATEWAY);
...
//         try {
//             String xml = new lottery.domains.pool.payment.zhf.util.HttpClientUtil().doPost(PayConfig.ZHF2.SCAN_GATEWAY, reqMap, "utf-8");		 	// 向智付发送POST请求
//             if (StringUtils.isEmpty(xml)) {
//                 log.error("智汇付2微信封装数据时返回空,订单号：" + billno);
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//
//             if (xmlResult == null) {
//                 log.error("智汇付2微信封装数据时解析空");
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"SUCCESS".equals(xmlResult.getRespCode())) {
//                 log.error("智汇付2微信封装数据时返回数据resp_code不是SUCCESS：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"0".equals(xmlResult.getResultCode())) {
//                 log.error("智汇付2微信封装数据时返回数据result_code不是0：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (StringUtils.isEmpty(xmlResult.getQrcode())) {
//                 log.error("智汇付2微信封装数据时返回二维码为空：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//             String xml = new lottery.domains.pool.payment.zhf.util.HttpClientUtil().doPost(PayConfig.ZHF2.SCAN_GATEWAY, reqMap, "utf-8");		 	// 向智付发送POST请求
//             if (StringUtils.isEmpty(xml)) {
//                 log.error("智汇付2支付宝封装数据时返回空,订单号：" + billno);
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//
//             if (xmlResult == null) {
//                 log.error("智汇付2支付宝封装数据时解析空");
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"SUCCESS".equals(xmlResult.getRespCode())) {
//                 log.error("智汇付2支付宝封装数据时返回数据resp_code不是SUCCESS：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"0".equals(xmlResult.getResultCode())) {
//                 log.error("智汇付2支付宝封装数据时返回数据result_code不是0：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (StringUtils.isEmpty(xmlResult.getQrcode())) {
//                 log.error("智汇付2支付宝封装数据时返回二维码为空：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//             return result;
//         } catch (Exception e) {
//             log.error("解析智汇付2扫码支付返回的xml时出错，订单号：" + billno, e);
//             e.printStackTrace();
//             return null;
//         }
//     }
...
//             String orginal_money = resMap.get("orginal_money"); // 该值为减额前的金额
//             //签名参数顺序不能乱
...
//             String signInfo = signStr.toString();
//             //智付公钥
//             if (RSAWithSoftware.validateSignByPublicKey(signInfo, PayConfig.ZHF2.PLATFORM_PUBLIC_KEY, dinpaySign)
//                     && "SUCCESS".equals(trade_status)) {
//                 RechargePay pay = new RechargePay();
//                 pay.setAmount(Double.parseDouble(order_amount));
//                 if (StringUtils.isNotEmpty(orginal_money)) {
//                     pay.setOrginalMoney(Double.parseDouble(orginal_money));
//                 }
//                 else {
//                     pay.setOrginalMoney(Double.parseDouble(order_amount));
//                 }
//                 SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
...
//                 return pay;
//             } else {
//                 log.warn("智汇付2回调验签失败，请求信息{}", JSON.toJSONString(resMap));
//                 return null;
//             }
//         } catch (Exception e) {
//             log.error("智汇付2回调发生异常，请求信息：" + JSON.toJSONString(resMap), e);
//         }
//         return null;
//     }
// }
