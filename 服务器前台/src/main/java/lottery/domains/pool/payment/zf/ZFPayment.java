// package lottery.domains.pool.payment.zf;
//
...
// import lottery.domains.pool.payment.zf.util.HttpClientUtil;
...
//     private static final Logger log = LoggerFactory.getLogger(ZFPayment.class);
...
//     private static final String SERVICE_TYPE_QQ_SCAN = "qq_scan";
...
//         try {
//             //商户公钥
//             sign = RSAWithSoftware.signByPrivateKey(signInfo, PayConfig.ZF.PRIVATE_KEY);
//         } catch (Exception e) {
//             e.printStackTrace();
//         }
//
...
//
//         result.setPayUrl(PayConfig.ZF.PAY_GATEWAY);
...
//         try {
//             String xml = new HttpClientUtil().doPost(PayConfig.ZF.SCAN_GATEWAY, reqMap, "utf-8");		 	// 向智付发送POST请求
//             if (StringUtils.isEmpty(xml)) {
//                 log.error("智付微信封装数据时返回空,订单号：" + billno);
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
//             ZFXMLResult xmlResult = resolveXMLResult(billno, xml);
//
//             if (xmlResult == null) {
//                 log.error("智付微信封装数据时解析空");
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"SUCCESS".equals(xmlResult.getRespCode())) {
//                 log.error("智付微信封装数据时返回数据resp_code不是SUCCESS：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"0".equals(xmlResult.getResultCode())) {
//                 log.error("智付微信封装数据时返回数据result_code不是0：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (StringUtils.isEmpty(xmlResult.getQrcode())) {
//                 log.error("智付微信封装数据时返回二维码为空：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//             String xml = new HttpClientUtil().doPost(PayConfig.ZF.SCAN_GATEWAY, reqMap, "utf-8");		 	// 向智付发送POST请求
//             if (StringUtils.isEmpty(xml)) {
//                 log.error("智付支付宝封装数据时返回空,订单号：" + billno);
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//
//             if (xmlResult == null) {
//                 log.error("智付支付宝封装数据时解析空");
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"SUCCESS".equals(xmlResult.getRespCode())) {
//                 log.error("智付支付宝封装数据时返回数据resp_code不是SUCCESS：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (!"0".equals(xmlResult.getResultCode())) {
//                 log.error("智付支付宝封装数据时返回数据result_code不是0：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//             if (StringUtils.isEmpty(xmlResult.getQrcode())) {
//                 log.error("智付支付宝封装数据时返回二维码为空：" + JSON.toJSONString(xmlResult));
//                 result.setReturnCode(PayConfig.wxPayReturnCode.payException);
//                 return;
//             }
//
...
//         Map<String, String> reqMap = new HashMap<String, String>();
//         reqMap.put("merchant_code", thridBean.getMerCode());
//         reqMap.put("service_type", SERVICE_TYPE_QQ_SCAN);
//         reqMap.put("notify_url", notifyUrl);
//         reqMap.put("interface_version", "V3.1");
//         reqMap.put("client_ip", ip);
//         reqMap.put("sign_type", "RSA-S");
...
//         signSrc.append("order_time=").append(orderTime).append("&");
//         signSrc.append("product_name=").append(productName).append("&");
//         signSrc.append("service_type=").append(SERVICE_TYPE_QQ_SCAN);
//
//         String signInfo = signSrc.toString();
//         String sign;	// 签名   signInfo签名参数排序，  merchant_private_key商户私钥
//         try {
...
//     }
//
//     private static ZFXMLResult resolveXMLResult(String billno, String xml) {
//         try {
...
//
//             ZFXMLResult result = new ZFXMLResult();
//
...
//             String signInfo = signStr.toString();
//             //智付公钥
//             if (RSAWithSoftware.validateSignByPublicKey(signInfo, PayConfig.ZF.PLATFORM_PUBLIC_KEY, dinpaySign)
...
//                 return pay;
//             } else {
//                 log.warn("智付回调验签失败，请求信息{}", JSON.toJSONString(resMap));
//                 return null;
//             }
//         } catch (Exception e) {
//             log.error("智付回调发生异常，请求信息：" + JSON.toJSONString(resMap), e);
//         }
//         return null;
//     }
// }
