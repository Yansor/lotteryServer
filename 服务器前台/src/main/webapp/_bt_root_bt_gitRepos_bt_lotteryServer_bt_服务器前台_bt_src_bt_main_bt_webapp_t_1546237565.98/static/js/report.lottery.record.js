$(function(){
    ...
    function initContentBox() {
        $.ajax({
            ...
            success: function(res) {
                $contentBox.html(tpl('#report_lottery_record_content_tpl', {lotteries: res.data}));
    ...
    function initDate() {
        ...
        $.YF.initDateUI('#reportLotteryRecordSTime');
        $.YF.initDateUI('#reportLotteryRecordETime');
    ...
    function search(page) {
        ...
        $.ajax({
            ...
            success : function(res) {
                if (res.error === 0) {
                    $listTable.html(tpl('#report_lottery_record_list_tpl', {rows: res.data}))
    ...
    // 撤销订单
    $contentBox.on('click', '[data-command="cancel"]', function(e){
        var id = $(e.target).closest('tr').attr('data-id');
