$(function(){
    ...
    function initContentBox() {
        $contentBox.html(tpl('#message_sys_content_tpl', {}));
        ...
        $.ajax({
            ...
            success : function(res) {
                if (res.error === 0) {
                    ...
                    $listTable.html(tpl('#sys_message_list_tpl', {rows: res.data}))
    ...
    function reviewMessageDetails(e) {
        ...
        if (cacheData[index].status === 0) {
            $.ajax({
                url: '/ReadUserSysMessage',
                data: {
                    ids: cacheData[index].id
                }
            })
            cacheData[index].status = 1;
            $target.closest('tr').removeClass("fwb");
            $target.parent().siblings("[data-type='status']").html("已读");
        }
        var data = cacheData[index];
        $.YF.alert_info(data.content);
    }
