$(function(){
    ...
    var $proxySalary = $("[name =proxy-salary]", $content);
    var $proxyDvidend = $("[name = proxy-dividend]", $content);
    ...
    function initdividendSettleInfo(){
        $.ajax({
            url: '/DividendSettleInfo',
            success: function(res) {
                if (res.error == 0 ) {
                	if(res.dividend == true){
                		$proxyDvidend.show();
                	}
                   	if(res.settle == true){
                   		$proxySalary.show();
