(function() {
    ...
    //转账中心控制器
    var transferAccountController = function($scope, shareDataService, postDataService, $state, $ionicPopup, $ionicLoading,passwordService) {
        var vm = this;
        vm.transObj = {
            userName: '',
            amount: '',
        ...
        vm.submitTransferAccount = function() {
            var userId="";

            if(!vm.transObj.userName){
                $ionicPopup.alert({
                    title: '温馨提示',
                    template: '用户名为空或不存在！'
                });
            ...
            var reg = /^[0-9]+$/ ;
            if (!reg.test(vm.transObj.amount)) {
                $ionicPopup.alert({
                    title: '温馨提示',
                    template: '转账金额请输入整数'
                });
            ...
            postDataService.submitData(loginData, '/transferSave').then(function(data) {
                data = data.data;
                if(data.code=='2-1009'){
                    $ionicPopup.alert({
                        title: '温馨提示',
	...
	angular.module('app.controllers', ['app.services'])
		...
		.controller('transferAccountController', transferAccountController)
