$(function(){
    ...
    function initContentBox() {
        $contentBox.html(tpl('#message_inbox_content_tpl', {}));
        ...
        $.ajax({
            ...
            success : function(res) {
                if (res.error === 0) {
                    ...
                    $listTable.html(tpl('#message_list_tpl', {rows: res.data, type: 'inbox'}))
    ...
    function reviewMessageDetails(e) {
        ...
        if (cacheData[index].bean.toStatus === 0) {
            $.ajax({
                url: '/ReadUserMessage',
                data: {
                    ids: cacheData[index].bean.id
                }
            })
            cacheData[index].bean.toStatus = 1;
            $target.closest('tr').removeClass("fwb");
            $target.parent().siblings("[data-type='status']").html("已读");
        }
        ...
        swal({
            ...
            self.window.location = '/message-new';
        }, function() {
        })
    }
