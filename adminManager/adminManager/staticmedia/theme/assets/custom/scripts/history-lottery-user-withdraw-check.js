var LotteryUserWithdrawCheck = function() {
	...
	var loadData = function() {
		if(!id) return;
		var url = './history-lottery-user-withdraw/check';
		var params = {id: id};
		$.ajax({
			type : 'post',
			url : url,
	...
	var buildWithdrawDetails = function(data) {
		...
		if(data.bean.status == 0 && data.bean.checkStatus == 0) {
			thisTable.find('[data-command="check"]').removeClass('hide');
		} else {
			thisTable.find('[data-command="check"]').addClass('hide');
		}
		
		thisTable.find('[data-command="check"]').unbind('click').click(function() {
		...
		thisTable.find('[data-command="cancel"]').unbind('click').click(function() {
		...
		if(data.length > 0) {
			...
			$.each(data, function(idx, val) {
				...
				'<tr class="align-center">'+
					...
					'<td>' + DataFormat.formatUserRechargeType(val.bean.type, val.bean.subtype) + '</td>'+
					'<td>' + val.bean.time + '</td>'+
