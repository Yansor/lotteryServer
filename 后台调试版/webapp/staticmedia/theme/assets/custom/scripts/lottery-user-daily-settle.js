var LotteryDailySettle = function() {
	var isLoading = false;
	// 最小销量值(单位W元)
	var minSale = 0;
	// 最大销量值(单位W元)
	...
	var UserDailySettleTable = function() {
		var tableList = $('#table-daily-settle-list');
		...
		var getSearchParams = function() {
			...
			return {username: username, sTime: sTime, eTime: eTime, minScale: minScale, maxScale: maxScale,
				minValidUser: minValidUser, maxValidUser: maxValidUser, status: status};
		}
		...
		var pagination = $.pagination({
			...
			pageSize: 20,
			ajaxType: 'post',
			ajaxUrl: './lottery-user-daily-settle/list',
			...
			success: function(list) {
				...
				$.each(list, function(idx, val) {
					var formatId = '<a href="javascript:;" data-val="' + val.username + '" data-command="showLowers">' + val.username + '</a>';
                    ...
                    if(val.status == 1){
                    	formatOperate = '<a data-command="del" data-username="' + val.username + '" href="javascript:;" class="btn default btn-xs black"><i class="fa fa-remove"></i> 删除</a>';
                    }
					innerHtml +=
...
//						'<td>' + minValidUser + '</td>'+
						...
						'<td>' + val.agreeTime + '</td>'+
						'<td>' + DataFormat.formatDailySettleStatus(val.status) + '</td>'+
						'<td>' + formatOperate + '</td>'+
						'</tr>';

				});
				table.html(innerHtml);
				table.find('[data-command="showLowers"]').unbind('click').click(function() {
					var upUsername = $(this).attr('data-val');
					resetParamsForUpUser(upUsername);
					UserDailySettleTable.init();
				});
				...
				table.find('[data-command="edit"]').unbind('click').click(function() {
					...
					loadEditDetails(id, function(data){
						if (data) {
							ModifyUserDailySettleModal.show(data);
						}
						else {
							bootbox.alert({
								title: "提示",
		...
		tableList.find('[data-command="add"]').unbind('click').click(function() {
			AddUserDailySettleModal.show();
		});

		var doDel = function(username) {
			var msg = '确认要删除会员<span class="fw600 fs14">【'+username+'】</span>的团队契约日结配置吗？（其所有下级也会被删除，该操作不可恢复！）';
			bootbox.dialog({
				message: msg,
				title: '提示消息',
				buttons: {
					success: {
						...
						callback: function() {
							...
							var url = './lottery-user-daily-settle/del';
							isLoading = true;
							$.ajax({
								type : 'post',
								url : url,
		...
		var loadEditDetails = function(id, callback) {
			var params = {id: id};
			var url = './lottery-user-daily-settle/edit-get';
			$.ajax({
				type : 'post',
				url : url,
				data : params,
	...
	var ModifyUserDailySettleModal = function() {
		...
		var doSubmit = function() {
			...
			var scale = form.find('input[name="scale"]').val();
			...
			var url = './lottery-user-daily-settle/edit';
			isSending = true;
			$.ajax({
				type : 'post',
				url : url,
		...
		var show = function(data) {
			if(data) {
				...
				if(data.upBean){
					...
					form.find('[data-field="upScale"]').html('<span class="static-placeholder-error"><i class="fa fa-warning"></i>上级招商未签署日结分红。</span>');
				}
	...
	var AddUserDailySettleModal = function() {
		var modal = $('#modal-add');
		var form = modal.find('form');
		...
		var initForm = function() {
			...
			modal.find('input[name="username"]').unbind('blur').blur(function() {
				...
				if(val && val.length >= 6) {
					...
					$.ajax({
						type : 'post',
						url : './lottery-user-daily-settle/add-get',
						data : {username: val},
						dataType : 'json',
						success : function(data) {
							...
							if(data.error === 0 && data.data) {
					            ...
					            if(data.data.upBean){
									...
									modal.find('[data-field="upScale"]').html('<span class="static-placeholder-error"><i class="fa fa-warning"></i>上级未签署日结分红。</span>');
								}
							}
							if(data.error == 1 || data.error == 2) {
		...
		var doSubmit = function() {
            ...
            if(!checkLevel(form)){
            	toastr['error']('契约日结错误，请参考各项条款最大最小值填写');
            	return;
            }
            ...
            for (var int = 0; int < losss.length; int++) {
				if(Number(losss[int])  > Number(sales[int])){
					toastr['error']('契约日结条款错误！条款中销量必须大于亏损', '操作提示');
					return;
				}
			}
//            var minValidUser = form.find('input[name="minValidUser"]').val();
			...
			var url = './lottery-user-daily-settle/add';
			isSending = true;
			$.ajax({
				type : 'post',
				url : url,
...
//				}
	...
	return {
		init: function() {
			UserDailySettleTable.init();
			ModifyUserDailySettleModal.init();
			AddUserDailySettleModal.init();
			handelDatePicker();
		}
	}
}();