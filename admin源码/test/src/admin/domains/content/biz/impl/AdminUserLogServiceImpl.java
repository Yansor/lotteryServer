...
public class AdminUserLogServiceImpl implements AdminUserLogService
{
    ...
    @Override
    public PageList search(final String username, final String ip, final String keyword, final String sDate, final String eDate, final int start, final int limit) {
        ...
        if (isSearch) {
            final List<AdminUserLogVO> list = new ArrayList<AdminUserLogVO>();
            final PageList pList = this.adminUserLogDao.find(criterions, orders, start, limit);
            for (final Object tmpBean : pList.getList()) {
                list.add(new AdminUserLogVO((AdminUserLog)tmpBean, this.adminDataFactory));
            }
            pList.setList(list);
            return pList;
        }
