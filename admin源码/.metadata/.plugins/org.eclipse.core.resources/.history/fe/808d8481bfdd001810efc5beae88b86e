...
@Controller
public class UserUpdateLogController extends AbstractActionController
{
    @Autowired
    ...
    @RequestMapping(value = { "/lottery-user-update-log/list" }, method = { RequestMethod.POST })
    @ResponseBody
    public void LOTTERY_USER_UPDATE_LOG(final HttpSession session, final HttpServletRequest request, final HttpServletResponse response) {
        final String actionKey = "/lottery-user-update-log/list";
        ...
        if (uEntity != null) {
            if (super.hasAccess(uEntity, actionKey)) {
                ...
                final PageList pList = this.adminUserCriticalLogService.search(username, start, limit);
                if (pList != null) {
