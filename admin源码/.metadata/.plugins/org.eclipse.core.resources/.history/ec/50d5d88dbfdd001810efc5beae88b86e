package lottery.domains.content.api.ag;

public class AGBetRecord
{
    private String dataType;
    private String billNo;
    private String playerName;
    private String agentCode;
    private String gameCode;
    private String netAmount;
    private String betTime;
    private String gameType;
    private String betAmount;
    private String validBetAmount;
    private String flag;
    private String playType;
    private String currency;
    private String tableCode;
    private String loginIP;
    private String recalcuTime;
    private String platformType;
    private String remark;
    private String round;
    private String result;
    private String beforeCredit;
    private String deviceType;
    
    public String getDataType() {
        return this.dataType;
    }
    
    public void setDataType(final String dataType) {
        this.dataType = dataType;
    }
    
    public String getBillNo() {
        return this.billNo;
    }
    
    public void setBillNo(final String billNo) {
        this.billNo = billNo;
    }
    
    public String getPlayerName() {
        return this.playerName;
    }
    
    public void setPlayerName(final String playerName) {
        this.playerName = playerName;
    }
    
    public String getAgentCode() {
        return this.agentCode;
    }
    
    public void setAgentCode(final String agentCode) {
        this.agentCode = agentCode;
    }
    
    public String getGameCode() {
        return this.gameCode;
    }
    
    public void setGameCode(final String gameCode) {
        this.gameCode = gameCode;
    }
    
    public String getNetAmount() {
        return this.netAmount;
    }
    
    public void setNetAmount(final String netAmount) {
        this.netAmount = netAmount;
    }
    
    public String getBetTime() {
        return this.betTime;
    }
    
    public void setBetTime(final String betTime) {
        this.betTime = betTime;
    }
    
    public String getGameType() {
        return this.gameType;
    }
    
    public void setGameType(final String gameType) {
        this.gameType = gameType;
    }
    
    public String getBetAmount() {
        return this.betAmount;
    }
    
    public void setBetAmount(final String betAmount) {
        this.betAmount = betAmount;
    }
    
    public String getValidBetAmount() {
        return this.validBetAmount;
    }
    
    public void setValidBetAmount(final String validBetAmount) {
        this.validBetAmount = validBetAmount;
    }
    
    public String getFlag() {
        return this.flag;
    }
    
    public void setFlag(final String flag) {
        this.flag = flag;
    }
    
    public String getPlayType() {
        return this.playType;
    }
    
    public void setPlayType(final String playType) {
        this.playType = playType;
    }
    
    public String getCurrency() {
        return this.currency;
    }
    
    public void setCurrency(final String currency) {
        this.currency = currency;
    }
    
    public String getTableCode() {
        return this.tableCode;
    }
    
    public void setTableCode(final String tableCode) {
        this.tableCode = tableCode;
    }
    
    public String getLoginIP() {
        return this.loginIP;
    }
    
    public void setLoginIP(final String loginIP) {
        this.loginIP = loginIP;
    }
    
    public String getRecalcuTime() {
        return this.recalcuTime;
    }
    
    public void setRecalcuTime(final String recalcuTime) {
        this.recalcuTime = recalcuTime;
    }
    
    public String getPlatformType() {
        return this.platformType;
    }
    
    public void setPlatformType(final String platformType) {
        this.platformType = platformType;
    }
    
    public String getRemark() {
        return this.remark;
    }
    
    public void setRemark(final String remark) {
        this.remark = remark;
    }
    
    public String getRound() {
        return this.round;
    }
    
    public void setRound(final String round) {
        this.round = round;
    }
    
    public String getResult() {
        return this.result;
    }
    
    public void setResult(final String result) {
        this.result = result;
    }
    
    public String getBeforeCredit() {
        return this.beforeCredit;
    }
    
    public void setBeforeCredit(final String beforeCredit) {
        this.beforeCredit = beforeCredit;
    }
    
    public String getDeviceType() {
        return this.deviceType;
    }
    
    public void setDeviceType(final String deviceType) {
        this.deviceType = deviceType;
    }
}
