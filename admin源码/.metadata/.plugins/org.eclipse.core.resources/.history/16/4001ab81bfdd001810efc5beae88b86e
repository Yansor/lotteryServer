...
public class UserBalanceSnapshotController extends AbstractActionController
{
    ...
    @RequestMapping(value = { "/user-balance-snapshot/list" }, method = { RequestMethod.POST })
    @ResponseBody
    public void USER_BALANCE_SNAPSHOT_LIST(final HttpSession session, final HttpServletRequest request, final HttpServletResponse response) {
        final String actionKey = "/user-balance-snapshot/list";
        ...
        if (uEntity != null) {
            if (super.hasAccess(uEntity, actionKey)) {
                ...
                final PageList pList = this.uBalanceSnapshotService.search(sDate, eDate, start, limit);
                if (pList != null) {
