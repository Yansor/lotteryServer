...
public class VerifyCodeUtils
{
    ...
    public static String generate(final String key, final HttpServletRequest request, final HttpServletResponse response) throws IOException {
        response.setContentType("image/jpeg");
        response.setHeader("Pragma", "no-cache");
        response.setHeader("Cache-Control", "no-cache");
        response.setDateHeader("Expires", 0L);
        final String code = outputVerifyImage(120, 40, (OutputStream)response.getOutputStream(), 6);
        request.getSession(true).setAttribute(key, (Object)code);
        return code;
    }
    
    public static String generateVerifyCode(final int verifySize) {
        return generateVerifyCode(verifySize, "1234567890");
    }
    ...
    public static void outputImage(final int w, final int h, final OutputStream os, final String code) throws IOException {
        ...
        for (int l = 0; l < verifySize; ++l) {
            ...
            affine.setToRotation(0.7853981633974483 * rand.nextDouble() * (rand.nextBoolean() ? 1 : -1), w / verifySize * l + fontSize / 2, h / 2);
    ...
    public static void main(final String[] args) throws IOException {
        ...
        for (int i = 0; i < 50; ++i) {
            ...
            final File file = new File(dir, String.valueOf(verifyCode) + ".jpg");
