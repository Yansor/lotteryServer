...
public class JedisShardedTemplate
{
    ...
    public JedisShardedTemplate(final JedisPool... jedisPools) {
        this.algo = Hashing.MURMUR_HASH;
        this.nodes = new TreeMap<Long, JedisTemplate>();
        this.singleTemplate = null;
        if (jedisPools.length == 1) {
            this.singleTemplate = new JedisTemplate(jedisPools[0]);
        }
        else {
            this.initNodes(jedisPools);
        }
    }
    
    public JedisShardedTemplate(final List<JedisPool> jedisPools) {
        this((JedisPool[])jedisPools.toArray(new JedisPool[jedisPools.size()]));
    }
    
    private void initNodes(final JedisPool... jedisPools) {
        for (int i = 0; i != jedisPools.length; ++i) {
            for (int n = 0; n < 128; ++n) {
                final JedisPool jedisPool = jedisPools[i];
                this.nodes.put(this.algo.hash("SHARD-" + i + "-NODE-" + n), new JedisTemplate(jedisPool));
            }
        }
    }
    
    public JedisTemplate getShard(final String key) {
        if (this.singleTemplate != null) {
            return this.singleTemplate;
        }
        final SortedMap<Long, JedisTemplate> tail = this.nodes.tailMap(this.algo.hash(key));
        if (tail.isEmpty()) {
    ...
    public <T> T execute(final String key, final JedisTemplate.JedisAction<T> jedisAction) throws JedisException {
        final JedisTemplate jedisTemplate = this.getShard(key);
        return jedisTemplate.execute(jedisAction);
    }
    
    public void execute(final String key, final JedisTemplate.JedisActionNoResult jedisAction) throws JedisException {
        final JedisTemplate jedisTemplate = this.getShard(key);
        jedisTemplate.execute(jedisAction);
    }
    
    public List<Object> execute(final String key, final JedisTemplate.PipelineAction pipelineAction) throws JedisException {
        final JedisTemplate jedisTemplate = this.getShard(key);
        return jedisTemplate.execute(pipelineAction);
    }
    
    public void execute(final String key, final JedisTemplate.PipelineActionNoResult pipelineAction) throws JedisException {
        final JedisTemplate jedisTemplate = this.getShard(key);
    ...
    public Boolean del(final String shardingKey, final String key) {
        final JedisTemplate jedisTemplate = this.getShard(shardingKey);
        return jedisTemplate.del(key);
    }
    ...
    public Double hincrByFloat(final String shardingKey, final String key, final String fieldName, final double increment) {
        final JedisTemplate jedisTemplate = this.getShard(shardingKey);
        return jedisTemplate.hincrByFloat(key, fieldName, increment);
    }
    ...
    public Set<Tuple> zrangeByScoreWithScores(final String shardingKey, final String key, final double min, final double max) {
        final JedisTemplate jedisTemplate = this.getShard(shardingKey);
        return jedisTemplate.zrangeByScoreWithScores(key, min, max);
    }
    ...
    public Set<Tuple> zrevrangeByScoreWithScores(final String shardingKey, final String key, final double max, final double min) {
        final JedisTemplate jedisTemplate = this.getShard(shardingKey);
        return jedisTemplate.zrevrangeByScoreWithScores(key, max, min);
    }
