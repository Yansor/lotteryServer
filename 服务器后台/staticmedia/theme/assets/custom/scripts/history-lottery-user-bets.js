...
var LotteryUserBets = function() {
	...
	var UserBetsTable = function() {
		...
		var getSearchParams = function() {
			...
			return {keyword: keyword, username: username, type: type, lotteryId: lotteryId, expect: expect, ruleId: ruleId,
		...
		var pagination = $.pagination({
			...
			pageSize: 20,
			ajaxType: 'post',
			ajaxUrl: './history-lottery-user-bets/list',
		...
		var doLoadDetails = function(id) {
			if(isLoading) return;
			var params = {id: id};
			var url = './history-lottery-user-bets/get';
			isLoading = true;
			$.ajax({
				type : 'post',
				url : url,
